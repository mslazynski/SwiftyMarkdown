...
open class SwiftyMarkdown {
    ...
    func attributedStringFromString(_ string : String, withStyle style : LineStyle, attributes : [String : AnyObject] = [:] ) -> NSAttributedString {
        ...
        // Use correct font name, if font name is available in style
        if let fontItalicName = italic.fontName, style == .italic {
            fontName = fontItalicName
        }
        else if let fontBoldName = bold.fontName, style == .bold {
            fontName = fontBoldName
        }
        else if let fontBoldItalicName = boldItalic.fontName, style == .boldItalic {
            fontName = fontBoldItalicName
        }
        
        let font = UIFont.preferredFont(forTextStyle: textStyle)
        let styleDescriptor = font.fontDescriptor
        let styleSize = fontSize ??
            styleDescriptor.fontAttributes[UIFontDescriptorSizeAttribute] as? CGFloat ??
            CGFloat(14)
